...
class SimpleParser {
    ...
    func parseDefinition() throws -> DefinitionNode {
        ...
        let returnTokens = consumeTo(.openBrace)
        let returnVal = reduce(returnTokens)
        
        let body = try parseToClosingBrace()
        return DefinitionNode(name: funcName, parameters: args, body: body, returnTypeString: returnVal)
    }
