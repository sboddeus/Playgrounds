 ...
 **Goal:** Create an epitrochoid.
 ...
 Yes! An epitrochoid is similar to a hypotrochoid, except the wheel rolls around the *outside* of the track (remember what the *epi-* prefix means). And, as with the hypotrochoid, its spoke length is flexible.
 
 * callout(Tip): 
 Right now, `wheelRadius` is set to a *positive* value, and you’re seeing a hypotrochoid. How do you need to edit the code to create an epitrochoid?
 */
