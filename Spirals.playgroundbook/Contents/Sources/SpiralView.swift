...
public class SpiralView: UIView {
    ...
    private var points: [CGPoint] = [] {
        didSet {
            ...
            points = points.enumerated().filter({ offsetElementPair in
                // Only include every other point.
                return offsetElementPair.offset % 2 != 0
            }).map({ offsetElementPair in
                // Map the enumerated tuple back to a single point.
                return offsetElementPair.element
            })
        }
    }
    ...
    public convenience init(frame: CGRect, roulette: Roulette) {
        ...
        layer.insertSublayer(wheelLayer, above: trackLayer)
    ...
    public override func draw(_ rect: CGRect) {
        ...
        cycloidPath.lineWidth = 1.5 / window!.screen.scale
        cycloidPath.stroke()
    }
    ...
    private func configureShapeLayers() {
        ...
        // radius trackRadius * scale, centered on the center point.
    ...
    // This represents how many points in a row we match up to check if we’ve completed the shape
    private let completionPointCount = 5
    ...
    func update() {
        ...
        if !isShapeComplete {
            ...
            if points.count > 1 && !cycloidPath.isEmpty {
                cycloidPath.addLine(to: currentNextPoint)
                
                // Make sure we have enough points in the array to check for completion
                guard points.count > countUp else { return }
                ...
                if sqrt((x * x) + (y * y)) < 2 && points.count > 200 {
                    ...
                    countUp = countUp + 1
                    if countUp == completionPointCount {
    ...
    /// This notifies the view that it has been rotated and partially needs to be redrawn, while keeping the cycloidPath
    func rotate() {
        configureShapeLayers()
        setNeedsDisplay()
    }
