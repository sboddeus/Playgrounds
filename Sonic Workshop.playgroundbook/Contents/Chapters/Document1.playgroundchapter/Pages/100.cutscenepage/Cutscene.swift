...
public class CaveOutroViewController: UIViewController {
    ...
    public override func viewDidLayoutSubviews() {
        let designedRatio = CGFloat(4.0/3.0)
        ...
        containerView.transform = CGAffineTransform.identity.scaledBy(x: properRatio, y: properRatio)
    ...
    private func zoomCrystals() {
        ...
        for i in 0...2 {
            UIView.animate(withDuration: 0.4, delay: initialDelay + delay, options: .curveEaseIn, animations: {
                ...
                crystal.transform = CGAffineTransform.identity.scaledBy(x: scale, y: scale)
                crystal.transform = crystal.transform.concatenating(CGAffineTransform.identity.translatedBy(x: offset.width, y: offset.height))
            }, completion: nil)
            delay += delayFactor
        }
    }
