...
public protocol ImageLearningBlockViewDelegate {
    func didLoadImage(imageBlockView: ImageLearningBlockView)
    func didTapFullscreenButton(imageBlockView: ImageLearningBlockView, screenRect: CGRect)
    func didTapHotspot(imageBlockView: ImageLearningBlockView, hotspot: LearningInteractive.Hotspot, screenRect: CGRect)
}
...
public class ImageLearningBlockView: UIView {
    ...
    public func load(url: URL) {
        ...
        let completion = {
            self.setNeedsLayout()
            self.layoutIfNeeded()
            self.delegate?.didLoadImage(imageBlockView: self)
    ...
    private func goFullScreen() {
        ...
        delegate?.didTapFullscreenButton(imageBlockView: self, screenRect: screenRect)
    }
    ...
    private func updateAX() {
        ...
        if isZoomed {
            zoomButton.accessibilityLabel = NSLocalizedString("Zoom out", comment: "AX label for zoom-out button in image block")
            zoomButton.accessibilityHint = NSLocalizedString("Makes the image smaller again.", comment: "AX hint for zoom-out button in image block.")
        } else {
            zoomButton.accessibilityLabel = NSLocalizedString("Zoom in", comment: "AX label for zoom-in button in image block")
            zoomButton.accessibilityHint = NSLocalizedString("Makes the image bigger.", comment: "AX hint for zoom-in button in image block.")
        }
    ...
    func didPressHotspotButton(_ sender: UIButton) {
        ...
        delegate?.didTapHotspot(imageBlockView: self, hotspot: hotspot, screenRect: screenRect)
    }
}
...
extension ImageLearningBlockView: LearningBlockViewable {
    public func load(learningBlock: LearningBlock, style: LearningBlockStyle, textStyle: AttributedStringStyle? = nil) {
        ...
        guard let name = learningBlock.attributes["source"] else { return }
        ...
        if let widthValue = learningBlock.attributes["width"] {
            if let width = Float(widthValue) {
                ...
                if let aspectRatioValue = learningBlock.attributes["aspect"] {
                    ...
                    // Display a warning.
                    let message = String(format: NSLocalizedString("⚠️ width (%@) specified without aspect: specify both, or height on its own.", comment: "Warning message for image block"), widthValue)
                    showMessage(message)
                }
            }
        }
        ...
        if name.hasSuffix(".png") {
            fileName = URL(fileURLWithPath: name).deletingPathExtension().lastPathComponent
        } else if name.hasSuffix(".pdf") {
            fileName = URL(fileURLWithPath: name).deletingPathExtension().lastPathComponent
            ext = "pdf"
        }

        guard let imageURL = Bundle.main.url(forResource: fileName, withExtension: ext) else { return }
