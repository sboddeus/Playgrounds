...
extension LearningBlockCell: ImageLearningBlockViewDelegate {
    ...
    public func didLoadImage(imageBlockView: ImageLearningBlockView) {
        setNeedsUpdateConstraints()
        delegate?.cellNeedsRefresh(self)
    }
    
    public func didTapFullscreenButton(imageBlockView: ImageLearningBlockView, screenRect: CGRect) {
        delegate?.cell(self, didRequestZoomImage: imageBlockView, at: screenRect)
    }
    
    public func didTapHotspot(imageBlockView: ImageLearningBlockView, hotspot: LearningInteractive.Hotspot, screenRect: CGRect) {
        ...
        let sourceRect = self.convert(screenRect, from: nil)
        ...
        HotspotViewController.present(attributedString: attributedText, from: currentViewController, sourceRect: sourceRect, sourceView: sourceView)
    }
}
