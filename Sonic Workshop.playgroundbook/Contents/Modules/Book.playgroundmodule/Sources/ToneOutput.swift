...
public class ToneOutput : AURenderCallbackDelegate {
    ...
    internal func performRender(_ ioActionFlags: UnsafeMutablePointer<AudioUnitRenderActionFlags>,
                       timeStamp: UnsafePointer<AudioTimeStamp>,
                       busNumber: UInt32,
                       numberFrames: UInt32,
                       ioData: UnsafeMutablePointer<AudioBufferList>) -> OSStatus
    {
        ...
        for i in 0 ..< frequencyCount {
            ...
            for frame in 0 ..< numberFrames {
                buffer[Int(frame)] = buffer[Int(frame)] + Float(sin(playThetas[i]) * tone.volume/Double(frequencyCount)) // frequencies are addative
