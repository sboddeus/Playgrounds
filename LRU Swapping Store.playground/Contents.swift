// A simple store that uses HD space as a backing store when there is memory pressure

// Reources:
// - Swift Algorithm Club for an LRU implementation
// - Use a tmp directory as should be provided by 

import Foundation

class LRUSwapDictionary<T: Codable, U: Hashable> {
    typealias Node = (el: T, key: U)
    let swap: SwapFileManager
    let list: LinkedList<Node>
    
    init(swap: SwapFileManager = UserDefaults(),
         notificationCenter: NotificationCenter = NotificationCenter.default) {
        self.swap = swap
        list = LinkedList<Node>()
        
        // Listen for notificaitons
        // PLaygrounds does not seem to have memory warning notifications
//          notificationCenter.addObserver(self, selector: #selector(memoryPressure), name: NSNotification.Name.didReceiveMemoryWarningNotification, object: nil)
    }
    
    func add(_ element: T, withKey key: U) {
        list.insert(Node(element, key), at: 0)
    }
    
    func get(_ key: U) -> T? {
        let index = list.firstIndex(where:  { (arg0) -> Bool in
            let (el, elKey) = arg0
            return key == elKey
        })
        if let safeIndex = index,
            let node = index?.node?.value { 
            // It was got, so move to head of list
            list.remove(at: safeIndex.tag)
            list.insert(node, at: 0)
            return node.el 
        } else {
            // check the swap
            let value: T? = swap.read(key: key)
            if let value = value {
                list.insert((value, key), at: 0)
                return value
            }
        }
    ...
    @objc func memoryPressure() {
        // Naive for now, move 12.5% of bottom nodes to storage
        // (always rounding to a minimum of 1)
        
        let count = list.count 
        guard count > 1 else { return }
        
        let swapOutCount = Int(Float(count) * (12.5/100))
        let roundedSwapOutCount = swapOutCount > 1 ? swapOutCount : 1
        
        for i in 0..<roundedSwapOutCount {
            let swapOut = list.removeLast()
            swap.write(swapOut.el, key: swapOut.key)
        }
    }
}

// Temp files
protocol SwapFileManager {
    func write(_ data: Data, key: String)
    func read(key: String) -> Data?
    func remove(key: String)
}

extension SwapFileManager {
    func write<T: Encodable, U: Hashable>(_ encodable: T, key: U) {
        let data = try! JSONEncoder().encode(encodable)
        self.write(data, key: "\(key.hashValue)")
    }
    func read<T: Decodable, U: Hashable>(key: U) -> T? {
        guard let data = self.read(key: "\(key.hashValue)") else { return nil }
        
        return try? JSONDecoder().decode(T.self, from: data)
    }
    func remove<U: Hashable>(key: U) {
        self.remove(key: "\(key)")
    }
}

extension UserDefaults: SwapFileManager {
    func write(_ data: Data, key: String) {
        set(data, forKey: key)
    }
...
// Linked List Implementation Take from Swift Algorithms Club
...
// ------ TESTS ----------
let swapDict = LRUSwapDictionary<[Int], String>()

let biggles = 1...10
let bigArray = Array(biggles)

for i in 1...6 {
    swapDict.add([i], withKey: "\(i)")
}
swapDict.add([10], withKey: "\(10)")
swapDict.add([11], withKey: "\(11)")
swapDict.add([12], withKey: "\(12)")

swapDict.list.description

swapDict.memoryPressure()
swapDict.memoryPressure()
